    ...
    if (response.statusCode == 200) {
      ...
      for (var category in data['trivia_categories']) {
        ...
        if (categoryName.contains('Vehicles')) {
          categoryGroups['Vehicles']?.add({'name': categoryName, 'id': categoryId});
        } else if (categoryName.contains('Computers')) {
          categoryGroups['Computers']?.add({'name': categoryName, 'id': categoryId});
        } else if (categoryName.contains('General Knowledge')) {
          categoryGroups['General Knowledge']?.add({'name': categoryName, 'id': categoryId});
        }else if (categoryName.contains('Video Games')) {
          categoryGroups['Video Games']?.add({'name': categoryName, 'id': categoryId});
        }else if (categoryName.contains('Sports')) {
          categoryGroups['Sports']?.add({'name': categoryName, 'id': categoryId});
        }else if (categoryName.contains('Mathematics')) {
          categoryGroups['Mathematics']?.add({'name': categoryName, 'id': categoryId});
        }
